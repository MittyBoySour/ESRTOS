...
static TPITChannelData PITChannelData[NB_SAMPLER_CHANNELS];
static TAnalyzerThreadData AnalyzerThreadData[NB_SAMPLER_CHANNELS];
static TAlarmMonitoringData AlarmMonitoringData[NB_SAMPLER_CHANNELS];
static TAlarmControlThreadData AlarmControlThreadData[NB_SAMPLER_CHANNELS];
...
static void InitSharedThreadData()
{
  ...
  PITData.PITFrequencySampleTakenSemaphore = OS_SemaphoreCreate(0);
  PITChannelData[NB_SAMPLER_CHANNELS];
  for (uint8_t channelNb = 0; channelNb < NB_SAMPLER_CHANNELS; channelNb++)
  {
  ...
  *PITData.PITChannelData = PITChannelData;
  ...
  for (uint8_t channelNb = 0; channelNb < NB_SAMPLER_CHANNELS; channelNb++)
  {
    ...
    AnalyzerThreadData[channelNb].analogSample = PITChannelData[channelNb].analogSample;
